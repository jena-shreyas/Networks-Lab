...
void* recv_message(void* sockfd){
    ...
    while(1){
        ...
        while(1){
            char len_buffer[4];
            int len_bytes_recv = 0;
            len_bytes_recv = recv(recv_sock, len_buffer, 4, 0);
            if (len_bytes_recv == 0){
            ...
            while (len_bytes_recv < 4)
            {
                len_bytes_recv += recv(recv_sock, len_buffer + len_bytes_recv, 4 - len_bytes_recv, 0);
                // printf("[ Recv Thread ]: Bytes received = %d\n", len_bytes_recv);
            }
            // printf("[ Recv Thread ]: Bytes received = %d\n", len_bytes_recv);
            
            length = 0;
            length = (len_buffer[0] - '0')*1000 + (len_buffer[1] - '0')*100 + (len_buffer[2] - '0')*10 + (len_buffer[3] - '0');
            // printf("[ Recv Thread ]: Length of the message = %ld\n", length);

            bytes_recv = recv(recv_sock, recv_buffer + total_bytes_recv, length - total_bytes_recv, 0);
            ...
            while (total_bytes_recv < length){
                bytes_recv = recv(recv_sock, recv_buffer + total_bytes_recv, length - total_bytes_recv, 0);
                total_bytes_recv += bytes_recv;
            }

            if (total_bytes_recv == length)
                break;
        ...
        int idx = (mysocket.recv_message->num_full + mysocket.recv_message->index ) % MAX_TABLE_SIZE;
        mysocket.recv_message->messages[idx].flag = 1;
        mysocket.recv_message->messages[idx].length = length;
        // mysocket.recv_message->messages[i].data = (char *)malloc((length)*sizeof(char));
        for (int j = 0; j < length; j++){
            mysocket.recv_message->messages[idx].data[j] = recv_buffer[j];
        }
        mysocket.recv_message->num_full++;
        // printf("[ Recv Thread ] Added the received message to the Receive Table at index %d!\n", idx);
...
int my_recv(int sockfd, void *buf, size_t len, int flags)
{
    ...
    int idx = mysocket.recv_message->index;
    size_t length = mysocket.recv_message->messages[idx].length;
    // printf("[my_recv]: Found a message to be read at index %d\n", idx);
    // printf("[my_recv]: Client Message Length: %ld\n", length);
    // printf("[my_recv]: Client Message: %s\n\n", mysocket.recv_message->messages[idx].data);
    ...
    for (int j = 0; j < length; j++){
        ((char *)buf)[j] = mysocket.recv_message->messages[idx].data[j];
    }

    // clear the recv table entry
    mysocket.recv_message->messages[idx].length = 0;
    mysocket.recv_message->messages[idx].flag = 0;
    // mysocket.recv_message->num_full--;
    memset(mysocket.recv_message->messages[idx].data, 0, MAX_MESSAGE_SIZE);

    // update the index
    mysocket.recv_message->index = (mysocket.recv_message->index + 1) % MAX_TABLE_SIZE;
    mysocket.recv_message->num_full--;
...
int my_socket(int domain, int type, int protocol)
{
    ...
    if (type == SOCK_MyTCP){
        ...
        mysocket.send_message->index = 0;
        mysocket.recv_message->index = 0;
